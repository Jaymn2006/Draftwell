...
import { auth as firebaseAuth, signInWithGoogle, logOutFirebase } from './lib/firebase'
  ...
  useEffect(() => {
    const unsub = firebaseAuth.onAuthStateChanged((fbUser) => {
      if (fbUser) {
        ...
        localStorage.setItem('draftwell-user-id', fbUser.uid)
        firebaseService.syncUserProfile(fbUser).catch(() => {})
      }
    })
    return () => unsub()
  }, [])
